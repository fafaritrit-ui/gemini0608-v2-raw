...
const AppProvider = ({ children }) => {
    ...
    const value = { db, auth, userId, userRole, username, isAuthReady, orders, products, expenses, users, storeSettings, currentPage, setCurrentPage, appId, handleLogout };
...
const OrdersPage = () => {
    const { db, appId, orders, products, userRole, userId, username, storeSettings } = useContext(AppContext);
    
    const initialOrderState = { customerName: '', items: [], totalCost: 0 };
    ...
    const handleEditOrder = (order) => {
        ...
        const { paidAmount, paymentStatus, paymentMethod, createdBy, ...rest } = order;
        setCurrentOrder({ 
            ...rest, 
    ...
    const handlePrintReceipt = (order) => {
        ...
        let receiptContent = `<div style="font-family: monospace; font-size: 10px; width: 80mm; text-align: left; padding: 5mm;">${storeSettings.logoUrl ? `<img src="${storeSettings.logoUrl}" alt="Logo" style="max-width: 100px; margin: 0 auto 10px auto; display: block;"/>` : ''}<h2 style="text-align: center; margin: 0; font-size: 14px;">${storeSettings.storeName || 'Toko Printing'}</h2><p style="text-align: center; margin: 2px 0;">${storeSettings.address || ''}</p><p style="text-align: center; margin: 2px 0 10px 0;">${storeSettings.phone || ''}</p><hr style="border-top: 1px dashed black;"><p><strong>ID:</strong> ${order.id}</p><p><strong>Nama:</strong> ${order.customerName}</p><p><strong>Tanggal:</strong> ${new Date(order.createdAt).toLocaleString('id-ID')}</p><p><strong>Kasir:</strong> ${order.createdBy?.username || 'N/A'}</p><hr style="border-top: 1px dashed black;"><p><strong>Detail:</strong></p><table style="width: 100%; font-size: 10px;">${parsedItems.map(item => { const product = products.find(p => p.id === item.productId); const itemPrice = calculateItemPrice(item); return `<tr><td colspan="3">${product ? product.name : 'N/A'}</td></tr><tr><td style="vertical-align: top;">${item.description || ''}</td><td style="text-align: right; vertical-align: top;">${item.quantity}x</td><td style="text-align: right; vertical-align: top;">Rp ${itemPrice.toLocaleString('id-ID')}</td></tr>`; }).join('')}</table><hr style="border-top: 1px dashed black;"><p><strong>Total:</strong> <span style="float: right;">Rp ${order.totalCost.toLocaleString('id-ID')}</span></p><p><strong>Dibayar:</strong> <span style="float: right;">Rp ${(order.paidAmount || 0).toLocaleString('id-ID')}</span></p><p><strong>Status:</strong> <span style="float: right;">${order.paymentStatus}</span></p><p><strong>Metode:</strong> <span style="float: right;">${order.paymentMethod || 'N/A'}</span></p>${change > 0 ? `<p><strong>Kembali:</strong> <span style="float: right;">Rp ${change.toLocaleString('id-ID')}</span></p>` : ''}${remaining > 0 ? `<p><strong>Sisa:</strong> <span style="float: right;">Rp ${remaining.toLocaleString('id-ID')}</span></p>` : ''}<hr style="border-top: 1px dashed black;"><p style="text-align: center; margin-top: 10px;">${storeSettings.receiptNotes || 'Terima kasih!'}</p></div>`;
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <form onSubmit={handleSubmitOrder} className="space-y-6">
                ...
                <div><h3 className="text-xl font-semibold mt-4 mb-2">Item Pesanan</h3>{currentOrder.items.map((item, index) => (<div key={index} className="grid grid-cols-1 md:grid-cols-5 gap-2 mb-2 p-2 bg-gray-50 rounded-lg border"><select value={item.productId} onChange={(e) => handleUpdateOrderItem(index, 'productId', e.target.value)} className="md:col-span-2 flex-grow p-2 border rounded-lg" required><option value="">Pilih Produk</option>{products.length > 0 ? products.map(product => (<option key={product.id} value={product.id}>{product.name} ({product.calculationMethod})</option>)) : (<option value="" disabled>Belum ada produk</option>)}</select>{products.find(p => p.id === item.productId)?.calculationMethod === 'dimensi' ? (<><input type="number" step="0.01" placeholder="Lebar (cm)" value={item.width || ''} onChange={(e) => handleUpdateOrderItem(index, 'width', parseFloat(e.target.value) || 0)} className="w-full p-2 border rounded-lg" required /><input type="number" step="0.01" placeholder="Tinggi (cm)" value={item.height || ''} onChange={(e) => handleUpdateOrderItem(index, 'height', parseFloat(e.target.value) || 0)} className="w-full p-2 border rounded-lg" required /></>) : (<input type="number" placeholder="Jumlah" value={item.quantity || ''} onChange={(e) => handleUpdateOrderItem(index, 'quantity', parseInt(e.target.value, 10) || 1)} className="w-full p-2 border rounded-lg" required />)}<input type="text" placeholder="Keterangan" value={item.description || ''} onChange={(e) => handleUpdateOrderItem(index, 'description', e.target.value)} className="md:col-span-2 w-full p-2 border rounded-lg" /><div className="flex items-center justify-between"><span className="text-sm font-semibold text-gray-700">Rp {calculateItemPrice(item).toLocaleString('id-ID')}</span><button type="button" onClick={() => handleRemoveOrderItem(index)} className="bg-red-500 text-white p-2 rounded-lg hover:bg-red-600">Hapus</button></div></div>))}<button type="button" onClick={handleAddOrderItem} className="mt-2 w-full bg-green-500 text-white p-2 rounded-lg hover:bg-green-600">Tambah Item</button></div>
                <div className="border-t pt-4">
                    <p className="text-xl font-bold text-right mb-4">Total Biaya: Rp {currentOrder.totalCost.toLocaleString('id-ID')}</p>
                </div>
            ...
            <div className="mt-8"><h2 className="text-2xl font-bold mb-4">Daftar Pesanan</h2><div className="overflow-x-auto"><table className="min-w-full bg-white rounded-lg shadow"><thead><tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal"><th className="py-3 px-6 text-left">ID</th><th className="py-3 px-6 text-left">Nama</th><th className="py-3 px-6 text-left">Total</th><th className="py-3 px-6 text-left">Dibayar</th><th className="py-3 px-6 text-left">Kekurangan</th><th className="py-3 px-6 text-left">Status</th><th className="py-3 px-6 text-left">Kasir</th><th className="py-3 px-6 text-left">Aksi</th></tr></thead><tbody className="text-gray-600 text-sm font-light">{orders.map(order => { const canEdit = userRole === 'owner' || userRole === 'superviser' || order.paymentStatus !== 'Lunas'; const deficiency = order.totalCost - (order.paidAmount || 0); return (<tr key={order.id} className="border-b border-gray-200 hover:bg-gray-100"><td className="py-3 px-6 text-left whitespace-nowrap">{order.id}</td><td className="py-3 px-6 text-left whitespace-nowrap">{order.customerName}</td><td className="py-3 px-6 text-left">Rp {order.totalCost.toLocaleString('id-ID')}</td><td className="py-3 px-6 text-left">Rp {(order.paidAmount || 0).toLocaleString('id-ID')}</td><td className="py-3 px-6 text-left text-red-600">Rp {deficiency > 0 ? deficiency.toLocaleString('id-ID') : 0}</td><td className="py-3 px-6 text-left"><span className={`py-1 px-3 text-xs font-bold rounded-full ${order.paymentStatus === 'Belum Lunas' ? 'bg-red-200 text-red-800' : 'bg-green-200 text-green-800'}`}>{order.paymentStatus}</span></td><td className="py-3 px-6 text-left">{order.createdBy?.username || 'N/A'}</td><td className="py-3 px-6 text-left"><button onClick={() => handleEditOrder(order)} disabled={!canEdit} className={`text-blue-500 hover:text-blue-700 mr-2 ${!canEdit && 'opacity-50 cursor-not-allowed'}`}>Edit</button><button onClick={() => handlePrintReceipt(order)} className="text-green-500 hover:text-green-700 mr-2">Cetak</button>{(userRole === 'superviser' || userRole === 'owner') && (<button onClick={() => setModal({ show: true, action: () => handleDeleteOrder(order.id), itemId: order.id })} className="text-red-500 hover:text-red-700">Hapus</button>)}</td></tr>);})}</tbody></table></div></div>
            ...
            try {
                ...
                items.forEach(item => {
                    ...
                    if (product) {
                        ...
                        sales[item.productId].quantity += (item.quantity || 1);
                        const price = (product.calculationMethod === 'dimensi') 
                            ? (item.width || 0) * (item.height || 0) * product.price
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg p-6 space-y-8">
            ...
            <div className="border-t pt-8">
                ...
                <button onClick={downloadItemSalesReport} className="mt-4 bg-green-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-600">Unduh Laporan Item</button>
            </div>
        </div>
    );
};
